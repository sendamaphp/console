...
namespace Sendama\Console\Debug;

use RuntimeException;
use Sendama\Console\Debug\Enumerations\LogLevel;
use Sendama\Console\Debug\Exceptions\DebugException;
use Sendama\Console\Util\Path;

final class Debug
{
    public const string DEFAULT_LOGS_DIR = 'logs';
    ...
    public static function getLogDirectory(): string
    {
        if (self::$logDirectory === null) {
            self::$logDirectory = Path::join(getcwd(), self::DEFAULT_LOGS_DIR);
        }
    ...
    ): void
    {
        ...
        if (!file_exists($filename)) {
            if (!is_writeable(self::getLogDirectory())) {
                throw new DebugException("The directory, " . self::getLogDirectory() . ", is not writable.");
            }

            if (false === $file = fopen($filename, 'w')) {
                throw new DebugException("Failed to create the debug log file.");
            }
            fclose($file);
        }
        ...
        if (false === error_log($message, 3, $filename)) {
            throw new DebugException("Failed to write to the debug log.");
        }
    }
    ...
    public static function error(string $message, string $prefix = '[ERROR]'): void
    {
        ...
        if (!file_exists($filename)) {
            ...
            if (false === $file = fopen($filename, 'w')) {
                throw new DebugException("Failed to create the error log file.");
            }
        ...
        $message = sprintf("[%s] %s - %s", date('Y-m-d H:i:s'), $prefix, $message) . PHP_EOL;
        if (false === error_log($message, 3, $filename)) {
            throw new DebugException("Failed to write to the error log.");
        }
    }
